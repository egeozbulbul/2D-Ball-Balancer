...
% 1) x variable (between 0-100): this is a fixed range!
x_vals        = 0:0.01:0.1;           % From 0 to 0.1 with step 0.01 (11 points)
xdot_vals     = -0.25:0.02:0.25;     
xdotdot_vals  = -3.2:0.2:3.2;   
phidot_vals   = -0.9:0.1:0.9;           
...
% Guaranteed Safe Set Calculation (intersection for all x values)
safe_matrix = ones(length(xdot_vals), length(xdotdot_vals), length(phidot_vals)); % initially all safe
...
save('safe_boundary_data.mat', 'safe_matrix', 'xdot_vals', 'xdotdot_vals', 'phidot_vals');
